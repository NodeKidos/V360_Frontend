...
export default function OtpVerification() {
  const [mode, setMode] = useState<OtpType>(OtpType.EMAIL);
  ...
  const onSubmit: SubmitHandler<FormData> = async (data) => {
    const target = mode === OtpType.EMAIL ? data.email! : data.phone!;
    const success = await generateOtp(mode, target);
      ...
      <div className="flex items-center justify-center p-5 sm:p-8 lg:p-10 z-10 relative">
        <Card className="w-full max-w-[500px] rounded-3xl shadow-md border border-gray-200 bg-white">
          <CardContent className="p-8 sm:p-10">
            ...
            <Tabs
              value={mode}
              onValueChange={(v) => setMode(v as OtpType)}
